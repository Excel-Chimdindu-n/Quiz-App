...
class QuestionList(LoginRequiredMixin, ListView):
    ...
    def get(self, request):
        quiz_profile = QuizProfile.objects.filter(user=request.user).annotate(
            total_score_annotation=Sum('total_score')).first()
...
def category_list(request):
    ...
    context = {'categories': categories, 'quiz_profile': quiz_profile}
...
def create_question(request):
    category_form = CategoryForm(author=request.user)
    question_form = QuestionForm(author=request.user)
        ...
        if 'create_category' in request.POST:
            ...
            if category_form.is_valid():
                ...
                category, created = Category.objects.get_or_create(
                    author=request.user, category=category_name)
                return redirect('quiz_new', category_id=category.id)
